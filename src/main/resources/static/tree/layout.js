...
export let familyChildrenLinkOffset = {};
...
export function getBlockId(blockRepresentantId) {
...
export function getBlock(blockRepresentantId) {
    return personsBlock[getBlockId(blockRepresentantId)].block;
}
function mergeBlocks(firstBlockRepresentantId, secondBlockRepresentantId) {
...
export function getSliceId(sliceRepresentantId) {
...
export function getSlice(sliceRepresentantId) {
    return personsSlice[getSliceId(sliceRepresentantId)].slice;
}
function mergeSlices(firstSliceRepresenantId, secondSliceRepresentantId) {
...
export function sliceToArray(sliceRepresentantId) {
    let result = [];
...
export function slicesInBlock(blockRepresentantId) {
    let result = [];
...
export function layerConstraintsToArray(layerIndex) {
    function blockToSlicedArray(blockRepresentantId) {
        let result = [];
...
export function recalculateConstraints() {
    ...
    function areInTheSameSlice(peopleIds) {
        if (peopleIds.size == 0) {
            return true;
        }
    ...
    function attemptConstraint(firstPersonId, secondPersonId, onlyBlock) {
        ...
        if (config.debug) {
            console.log("Attempting to constrain [" + firstPersonId + "," + secondPersonId + "]");
        }
        // We don't constrain partners across layers
        if (personsLayer[firstPersonId] != personsLayer[secondPersonId]) {
            if (config.debug) {
                console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: different layer");
            }
            return false;
        }
        ...
        if (getBlockId(firstPersonId) == getBlockId(secondPersonId)) {
            if (config.debug) {
                console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: already in the same block");
            }
            return false;
        }
        if (firstBlock.peekRight() != firstPersonId) {
            if (config.debug) {
                console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: " + firstPersonId + " is not at the right edge of the block");
            }
            return false;
        }
        if (secondBlock.peekLeft() != secondPersonId) {
            if (config.debug) {
                console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: " + secondPersonId + " is not at the left edge of the block");
            }
            return false;
        }
        ...
        if (firstConstraints.assignedFamily != undefined && secondConstraints.assignedFamily != undefined) {
            ...
            if (firstFamilySlice == undefined) {
                if (config.debug) {
                    console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: family " + firstConstraints.assignedFamily + " has no parents slice.");
                }
                return false;
            }
            ...
            for (const familyId of personsConstraints[firstFamilySlice.right].endsFamilySlices) {
                if (familyAssignedChildren[familyId].length < familyAssignedChildren[firstConstraints.assignedFamily].length) {
                    if (config.debug) {
                        console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: link would collide with other for the person on the left.");
                    }
                    return false;
                }
            }
            for (const familyId of personsConstraints[secondFamilySlice.left].beginsFamilySlices) {
                if (familyAssignedChildren[familyId].length < familyAssignedChildren[secondConstraints.assignedFamily].length) {
                    if (config.debug) {
                        console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: link would collide with other for the person on the right.");
                    }
                    return false;
                }
            }
            if (firstConstraints.lockedChild && familyAssignedChildren[firstConstraints.assignedFamily].slice(-1)[0] != firstPersonId) {
                if (config.debug) {
                    console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: " + firstPersonId + " is already locked in place and can't be moved to the end of its family.");
                }
                return false;
            }
            if (secondConstraints.lockedChild && familyAssignedChildren[secondConstraints.assignedFamily][0] != secondPersonId) {
                if (config.debug) {
                    console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: " + secondPersonId + " is already locked in place and can't be moved to the beginning of its family.");
                }
                return false;
            }
            if (!attemptConstraint(firstFamilySlice.right, secondFamilySlice.left, "only-block")) {
                if (config.debug) {
                    console.log("Can't constrain [" + firstPersonId + "," + secondPersonId + "]: parents cannot be constrained.");
                }
                return false;
            }
    ...
    for (const layer of layers) {
        for (const personId of layer) {
            ...
            if (bestFamilyForPerson != undefined) {
                ...
                personsConstraints[personId].beginsFamilySlices.sort((a, b) => model.familyParents(a).length - model.familyParents(b).length);
                personsConstraints[personId].endsFamilySlices.sort((a, b) => model.familyParents(b).length - model.familyParents(a).length);
...
export function recalculateLayout() {
    ...
    function lastPersonInLayoutLayer(layer) {
    ...
    function pushSliceIntoLayout(sliceId) {
        if (personsLayoutPosition[sliceId] != undefined) {
            return;
        }
        let slice = getSlice(sliceId);
        const layer = personsLayer[slice.left];
        let finishedUnhookedFamilies = [];
        let finishedFamiliesWithinSlice = {};
        let partners = [];
        let families = [];
        ...
        for (const familyInfo of finishedUnhookedFamilies) {
            ...
            const nodeLayoutPosition = { layer: layer, position: layout[layer].length };
    ...
    for (const layerIndex in layers) {
        ...
        for (const parentlessFamilyId of parentlessFamiliesInLayer[layerIndex]) {
            const nodeLayoutPosition = { layer: +layerIndex, position: layout[layerIndex].length };
...
export function recalculatePositions() {
    ...
    familyPosition = {};
    let layerBox = [];
    let nextLayoutNodeToDrawOnLayer = [];
    ...
    function calculatePositionForPerson(personId, suggestedBoxStart) {
        ...
        if (config.debug) {
            console.log("Ending person " + personId + ": " + boxStart + " [" + layerBox[personsLayer[personId]] + "]");
        }
        return [boxStart, boxStart];
    }
    function calculatePositionForFamilyMembers(members, boxStart) {
        if (config.debug) {
    ...
    function calculatePosition(node, suggestedBoxStart) {
        if (node.kind == "people") {
            if (config.debug) {
            ...
            if (realBoxStart == null) {
                realBoxStart = suggestedBoxStart;
            }
            ...
            for (const personId of node.partners) {
                if (!first) {
                    partnersBoxEnd += spaceBetweenPeople;
                }
                else {
                    first = false;
                }
    ...
    function calculateLayoutNode(position, suggestedBoxStart) {
        if (config.debug) {
